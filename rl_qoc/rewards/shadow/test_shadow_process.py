...
"""
# 2 qubit circuit of one parameter
...
# build the choi matrix which serves as the target
...
params = np.array([param_random, param_random - 0.5, param_random + 0.5]) # , param_random - 1, param_random + 1])
...
execution_config = ExecutionConfig(batch_size=batch_size,
                                   sampling_paulis=shadow_size, # this parameter is currently not used 
                                   n_shots=1,
                                   seed=42
                                   )
reward = ShadowReward()
