...
class CalibrationEstimatorPub(EstimatorPub):
    ...
    def __init__(
        ...
        if input_states is None:
            ...
            isinstance(input_state, QuantumCircuit) for input_state in input_states
        ):
    ...
    def to_pub_list(self) -> List[EstimatorPub]:
        ...
        for input_state in self.input_states:
            ...
            pub_list.append(
                EstimatorPub(
                    circuit, self._observables, self._parameter_values, self._precision
                )
            )
    ...
    def validate(self):
        ...
        if not isinstance(self._input_states, list):
            raise TypeError(
                f"input_states must be a list, not {type(self._input_states)}"
            )
        if not all(
            isinstance(input_state, QuantumCircuit)
            for input_state in self._input_states
        ):
            raise TypeError("All elements of input_states must be QuantumCircuits.")

    @classmethod
    def coerce(
        cls, pub: CalibrationEstimatorPubLike, precision: float | None = None
...
class CalibrationSamplerPub(SamplerPub):
    ...
    def to_pub_list(self) -> List[SamplerPub]:
        ...
        for input_state in self.input_states:
            ...
            if input_state.data:
                circuit.compose(input_state, front=True, inplace=True)
            pub_list.append(SamplerPub(circuit, self._parameter_values, self._shots))
        return pub_list
    ...
    @classmethod
    def coerce(
        cls, pub: CalibrationSamplerPubLike, shots: int | None = None
        ...
        if isinstance(pub, CalibrationSamplerPub):
            ...
            if pub.precision is None and shots is not None:
                return cls(pub.circuit, pub.parameter_values, shots, validate=False)
        ...
        return cls(
            circuit, parameter_values, shots, n_reps, input_states, validate=True
        )
...
CalibrationEstimatorPubLike = Union[
    EstimatorPubLike,
    Tuple[
        QuantumCircuit,
        ObservablesArrayLike,
        ...
        Union[Iterable[QuantumCircuit], None],
    ],
]

CalibrationSamplerPubLike = Union[
    SamplerPubLike,
    Tuple[
        QuantumCircuit,
        BindingsArrayLike,
