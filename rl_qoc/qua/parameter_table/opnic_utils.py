...
def make_cpp_packet_definition(direction: Literal["Incoming", "Outgoing"],
                               param_table: Union[ParameterTable, Parameter]) -> str:
    ...
    for i, parameter in enumerate(parameters):
        packet_definition += (f"({parameter.name}, {get_cpp_type(parameter)}, "
...
def make_h_packet_definition(direction: Literal["Incoming", "Outgoing"],
                             param_table: Union[ParameterTable, Parameter]) -> str:
...
def make_i_packet_definition(direction: Literal["Incoming", "Outgoing"],
                             param_table: Union[ParameterTable, Parameter]) -> str:
    """ Constructs the C++ header code defining the QM packet structure. """
    struct_name = f"{direction}Packet"
...
def patch_opnic_wrapper(param_tables: List[Union[ParameterTable, Parameter]],
    ...
    for param_table in param_tables:
        direction: Literal["Incoming", "Outgoing"] = "Incoming" if param_table.direction == "INCOMING" else "Outgoing"
        ...
        typecast = make_outgoing_packet_typecast(param_table) if direction == "Outgoing" else make_ingoing_packet_typecast(param_table)
