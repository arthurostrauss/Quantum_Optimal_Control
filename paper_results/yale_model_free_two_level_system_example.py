...
seed = 2763  # Seed for action sampling
...
_, log_probs = Normal_distrib.experimental_sample_and_log_prob(sample_shape=[batchsize], seed=seed)
...
for i in tqdm(range(n_epochs)):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        """
        Normal_distrib = Normal(loc=mu, scale=tf.abs(sigma) + sigma_eps, allow_nan_stats=False)
        a2, log_probs = Normal_distrib.experimental_sample_and_log_prob(sample_shape=[batchsize], seed=seed)
        ...
        if insert_baseline:
            ...
            if concurrent_optimization:
                ...
                combined_loss2 = actor_loss2 + critic_loss2
