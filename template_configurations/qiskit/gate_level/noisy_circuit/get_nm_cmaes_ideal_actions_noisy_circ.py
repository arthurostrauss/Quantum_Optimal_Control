...
def get_summary(optimized_fidelity, optimal_noise_free_params, optimized_params, optimal_deviations, end_time):
    return {
...
def get_cma_es_result(optimal_noise_free_params, phi_val, gamma_val, backend):
    ...
    """
    initial_sigma = 0.1
    es = cma.CMAEvolutionStrategy(optimal_noise_free_params, 0.1, {'maxiter': 1000, 'tolx': 1e-6})
    ...
    optimized_fidelity = -result.best.f  # Negate because fidelity_function returns negative fidelity
    optimal_deviations = optimal_noise_free_params - optimized_params

    return get_summary(optimized_fidelity, optimal_noise_free_params, optimized_params, optimal_deviations, end_time)
...
def get_optimized_params(optimal_noise_free_params, phi_val, gamma_val, backend) -> dict:
    ...
    return {
        ...
        'uncorrected_gate_fidelity': -fidelity_function(optimal_noise_free_params, phi_val, backend),
...
def main(optimal_noise_free_params, phis, gammas, save_file_name: Optional[str] = None) -> dict:
    ...
    for phi_val in phis:
        for gamma_val in gammas:
            ...
            logging.warning('Parameter Pair: phi = {}pi; gamma = {}'.format(phi_val / np.pi, gamma_val))

            backend = get_noisy_backend(gamma_val, phi_val)
            result[key] = get_optimized_params(optimal_noise_free_params, phi_val, gamma_val, backend)
