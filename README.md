...
- ```n_qubits: int```: the number of qubits composing our quantum system of interest, this should be an integer
- ``` target: Dict```: a dictionary containing information about the target that the RL agent should prepare. It could either be a target state or a target gate. This dictionary should contain the following keys:
    ...
    - ```circuit: qiskit.opflow.CircuitOp```: If target type is a state, provides an ideal quantum circuit that should prepare the target state of interest
    ...
    - ```register: List[int]```: List of qubits on which the target state/gate shall be prepared, could be a subset of the qubits forming the quantum system. The indices provided in this list, as well as its length, should be below or equal to ```n_qubits```.
    - ```input_states: List[Dict[Union["circuit", "dm"]: Union[qiskit.opflow.CircuitOp, qiskit.quantum_info.DensityMatrix]]]```: List of possible input states that should be prepared for forcing the agent to perform simultaneous successful state preparation. In order to maximize learning, the set of input states should be tomographically complete. Each input state should be provided as if it was a target state (either provide a quantum circuit enabling its perfect preparation,  or a DensityMatrix)
- ```abstraction_level: str = Union["gate", "pulse"]```: The abstraction level at which the parametrized circuit enabling the target preparation is expressed. If the gate level is chosen, then the simulator that is used is the traditional ```QasmSimulator()``` Qiskit backend, and significantly speeds up the simulation. Contrariwise, if the pulse level is chosen, all gates provided in the parametrized circuit should contain a dedicated pulse description, and the simulation of the pulse schedule associated to the circuit is done through Qiskit Dynamics module (https://qiskit.org/documentation/dynamics). 
- ```Qiskit_config: Dict```: A dictionary containing necessary information to execute the algorithm on real/simulated hardware. It should contain the following keys:
    - ```backend: Optional[Union[qiskit.providers.BackendV1, qiskit.providers.BackendV2]]```: An IBM Backend on which should be executed the overall workflow. If a real backend is selected, then the user should ensure that he has opened an IBM account to access the resource, and that this backend is selected among the ones enabling the execution of Qiskit Runtime (see code examples about how to declare such real backend). If the ```abstraction_level```is set to ```"gate"```, and the user wants to run on a simulated noiseless backend, he can set the ```backend```to ```None```. Finally, if the user wants to run the workflow on a simulated backend at the pulse level, he should provide a ```qiskit_dynamics.DynamicsBackend``` instance (see pulse level abstraction folder).
