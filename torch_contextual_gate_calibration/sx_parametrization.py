...
def custom_sx_schedule(backend: Backend, physical_qubits=Union[int, tuple, list], params: ParameterVector=None, n_actions: int=4):
    """
    ...
    print('New Pulse Parameters', new_params)
...
def get_target_gate(gate: Gate, register: QuantumRegister):
...
def get_target_info():
    ...
    n_actions = 4 # number of parameters for the SX gate: amplitude, sigma, beta and duration
    params = ParameterVector('theta', n_actions)

    return target, n_actions, params, physical_qubits
...
def add_parametrized_circuit(qc: QuantumCircuit, params: Optional[ParameterVector]=None, tgt_register: Optional[QuantumRegister]=None):
    ...
    target, n_actions, params, physical_qubits = get_target_info()
    ...
    parametrized_schedule = custom_sx_schedule(backend=fake_backend, physical_qubits=physical_qubits, params=params, n_actions=n_actions)
    qc.add_calibration(parametrized_gate, physical_qubits, parametrized_schedule)
    qc.append(parametrized_gate, tgt_register)


def get_circuit_context(fake_backend):
    ### Quantum Circuit Context (What happens around our specified target gate)
    ...
    return target_circuit, transpiled_circ

def own_hamiltion(qubit_properties):
...
def db_from_backend(fake_backend, physical_qubits, qubit_properties, dynamics_options):
...
def get_quantum_environment(fake_backend):

    _, _, _, physical_qubits = get_target_info()
    ...
    # Get a dynamics backend object
    dynamics_backend = db_from_backend(fake_backend, physical_qubits, qubit_properties, dynamics_options)
    ...
    target, _, _, physical_qubits = get_target_info()
    ...
    return q_env, dynamics_backend

q_env, dynamics_backend = get_quantum_environment(fake_backend)
dt, coupling_map, basis_gates, instruction_durations = retrieve_backend_info(backend=dynamics_backend)

def get_model(device, observation_space, n_actions):
...
def get_torch_qe(device, training_steps_per_gate, benchmark_cycle, batchsize, n_actions, min_bound_actions, max_bound_actions):
    seed = 10
    ...
    target_circuit, transpiled_circ = get_circuit_context(fake_backend)
    ...
    agent, actor_net, critic_net = get_model(device, observation_space, n_actions)
    
    return torch_env, agent, actor_net, critic_net, tgt_instruction_counts, seed


run_name = "test"
writer = SummaryWriter(f"runs/{run_name}")

def build_empty_train_arr(torch_env, tgt_instruction_counts, batchsize, device):
    global_step = 0
    ...
    return global_step, obs, actions, logprobs, rewards, dones, values, train_obs

def reset_environment(torch_env, batchsize, device, global_step, seed):
    next_obs, _ = torch_env.reset(seed=seed)
    num_steps = torch_env.episode_length(global_step)
    next_obs = torch.Tensor(np.array([next_obs] * batchsize)).to(device)
    next_done = torch.zeros(batchsize).to(device)
    return next_obs, num_steps, next_done

def update_step(next_obs, max_bound_actions, scale_factor, min_bound_actions, step, device, batchsize, num_steps, global_step, torch_env, agent, values, actions, logprobs, rewards, gamma, gae_lambda, epsilon, ent_coef, critic_loss_coeff, grad_clip, optimizer, minibatch_size, normalize_advantage, clip_vloss, clip_coef, n_epochs):
    with torch.no_grad():
        # Calculate mean action, standard deviation, and critic value
    ...
    # Add scalar information to TensorBoard
    writer.add_scalar("charts/episodic_return", np.mean(reward), global_step)
    writer.add_scalar("charts/episodic_length", num_steps, global_step)
    
    return next_obs, rewards, done, mean_action, std_action, next_done

def calculate_advantages(agent, device,num_steps, dones, next_done, next_obs, rewards, values, gamma, gae_lambda):
...
def flatten_batch(torch_env, obs, logprobs, actions, advantages, returns, values, batchsize):
...
def optimize_policy_and_value(torch_env, n_epochs, global_step, agent, obs, logprobs, actions, advantages, returns, values, batchsize, minibatch_size, epsilon, ent_coef, critic_loss_coeff, grad_clip, optimizer, normalize_advantage, clip_vloss, clip_coef, mean_action, std_action):
    # flatten the batch
    b_obs, b_logprobs, b_actions, b_advantages, b_returns, b_values, b_inds = flatten_batch(torch_env, obs, logprobs, actions, advantages, returns, values, batchsize)
    clipfracs = []
    ...
    return {
            'Average return:': np.mean(torch_env.reward_history, axis=1)[-1],
            'Circuit fidelity': torch_env.circuit_fidelity_history[-1],
            'Avg gate fidelity': torch_env.avg_fidelity_history[-1],
            'Mean vector': mean_action[0],
            'Sigma vector': std_action[0],
        }