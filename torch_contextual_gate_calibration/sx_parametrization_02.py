...
dynamics_backend, Qiskit_setup, q_env = get_db_qiskitconfig()
...
actor_net, critic_net, agent = get_network(device)
...
n_epochs, num_updates, lr_actor, lr_critic, epsilon, critic_loss_coeff, optimizer, actor_optimizer, critic_optimizer, minibatch_size, gamma, gae_lambda, clip_vloss, grad_clip, clip_coef, normalize_advantage, ent_coef = get_hyperparams()
...
global_step, obs, actions, logprobs, rewards, dones, values, train_obs, visualization_steps = clear_history()

# %%
def train_agent(global_step):
